...
Str1 strcpy12 (Str1 out, const_Str2 in)
{
    ...
    while(!strEnd2(in))
    {
        ...
        else if(c<=0x07FF){ *out++ = 0xC0|((c>>6*1)&0x1F);
                            *out++ = 0x80|((c>>6*0)&0x3F); }

        else if(c<=0xFFFF){ *out++ = 0xE0|((c>>6*2)&0x0F);
                            *out++ = 0x80|((c>>6*1)&0x3F);
                            *out++ = 0x80|((c>>6*0)&0x3F); }

        #if __WCHAR_MAX__ > 0x10000
        else { if(c>0x10FFFF) c = 0x10FFFF;
                            *out++ = 0xF0|((c>>6*3)&0x07);
                            *out++ = 0x80|((c>>6*2)&0x3F);
                            *out++ = 0x80|((c>>6*1)&0x3F);
                            *out++ = 0x80|((c>>6*0)&0x3F); }
...
Str2 strcpy21 (Str2 out, const_Str1 in)
{
    ...
    while(!strEnd1(in))
    {
        ...
        {
             if(i==2) c = ((in[0]&0x1F)<<6*1)
                        | ((in[1]&0x3F)<<6*0);

        else if(i==3) c = ((in[0]&0x0F)<<6*2)
                        | ((in[1]&0x3F)<<6*1)
                        | ((in[2]&0x3F)<<6*0);

        #if __WCHAR_MAX__ > 0x10000
        else if(i==4) c = ((in[0]&0x07)<<6*3)
                        | ((in[1]&0x3F)<<6*2)
                        | ((in[2]&0x3F)<<6*1)
                        | ((in[3]&0x3F)<<6*0);
        #endif
